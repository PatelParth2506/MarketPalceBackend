...
const ctrlAddDiscount = async (req, res) => {
  ...
  if((req.body.typeOfDiscount === 'per' && req.body.discount > 100 ) || ( req.body.typeOfDiscount === 'flat' && req.body.discount > product.price )) throw new apiError(HTTP_STATUS.BAD_REQUEST,"You Cant Give More Discount Then Product Price")
  
  const overlapDiscount = await Discount.findOne({ where : {
      product_id: req.body.product_id ,
      [Op.or]:[{
        startingDate :{
  ...
  if(overlapDiscount) throw new apiError(HTTP_STATUS.BAD_REQUEST,"Discount Already Exists In This Time")
...
const ctrlDeleteDiscount = async (req, res) => {
  ...
  if (!discount) throw new apiError(HTTP_STATUS.NOT_FOUND, "No Discount Found");
...
const ctrlGetDiscountOfProduct = async(req,res)=>{
    ...
    const discount = await Discount.findAll({ where:{ product_id : req.body.product_id } })
    if(!discount) throw new apiError(HTTP_STATUS.NOT_FOUND,"No Discount Found with This ProductId")
...
const ctrlUpdateDiscount = async (req, res) => {
  ...
  if (req.body.discount) {
    if((discount.typeOfDiscount === 'per' && req.body.discount > 100 ) || ( discount.typeOfDiscount === 'flat' && req.body.discount > discount.Products.price )) throw new apiError(HTTP_STATUS.BAD_REQUEST,"Discount Can't Higher Then Product Price")
    updatedData.discount = req.body.discount
}
  if (req.body.product_id) updatedData.product_id = req.body.product_id;
