...
const ctrlCreateCategory = async (req, res) => {
  ...
  if (categorycheck)
    throw new apiError(HTTP_STATUS.BAD_REQUEST, "Category With This Creditinals Already Exists");
  const image_path = req.file.path;
  if (!image_path)
    throw new apiError(HTTP_STATUS.BAD_REQUEST, "Image Path Is Required To Upload Image");
  ...
  res
    ...
    .json(
      new apiResponse(HTTP_CODE.CREATED, category, "Category Created SuccessFully")
    );
};
...
const ctrlGetCategory = async (req, res) => {
  ...
  if (!categorys) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND, "No Records Found");
  res.status(HTTP_STATUS.OK).json(new apiResponse(HTTP_CODE.OK, categorys, "All Data Fetched"));
};

const ctrlUpdateCategory = async (req, res) => {
  const category = await Category.findByPk(req.body.id);
  if (!category) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND, "No Records Found");
  ...
  if (JSON.stringify(updatedData) === "{}") {
    throw new apiError(HTTP_STATUS.BAD_REQUEST, "One of the fields must be updated");
  }
  ...
  res
    ...
    .json(new apiResponse(HTTP_CODE.OK, category, "Data Updated Successfully", true));
};
...
const ctrlDeleteCategory = async (req, res) => {
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(new apiResponse(HTTP_CODE.OK, [], "Records Deleted SuccessFully", true));
};
...
const ctrlGetSingleCategory = async (req, res) => {
  ...
  if (!category) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND, "No Category Found");
  ...
  res
    .status(HTTP_STATUS.OK)
    .json(new apiResponse(HTTP_CODE.OK, "Category Fetched SuccessFully", newdata));
};
