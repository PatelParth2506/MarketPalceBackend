...
const ctrlCreateProduct = async(req,res)=>{
    ...
    if(productCheck) throw new apiError(HTTP_STATUS.BAD_REQUEST,"Product With This Creditinsals Already Exists")
        
    const subcategory = await Subcategory.findOne({where:{ id:req.body.subcategory_id }})
    if(!subcategory) throw  new apiError(HTTP_STATUS.DATA_NOT_FOUND,"Invalid Subcategory")
    ...
    if(!product) throw new apiError(HTTP_STATUS.INTERNAL_SERVER_ERROR,"Error Createing Product")
    let productImage;
    if(req.files && req.files.length > 0){
         productImage = req.files.map((file)=>{
            if(file.mimetype !== 'image/png' && file.mimetype !== 'image/jpeg' && file.mimetype !== 'image/jpg') throw new apiError(HTTP_STATUS.VALIDATION,"Only Png,Jpg And Jpeg Are Allowed")
            return{
    ...
    res.status(HTTP_STATUS.CREATED).json(
        new apiResponse(HTTP_STATUS.CREATED,"Product Created SuccessFully",[product,productImages])
    )
}
...
const ctrlDeleteProduct = async(req,res)=>{
    ...
    if(!product) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND,"No Product Found With This ID")
    
    if(req.user.id !== product.id && req.user.role === 'user') throw new apiError(HTTP_STATUS.UNAUTHORIZED,"Unauthorized Access")
    
    product.is_delete = true
    await product.save()

    res.status(HTTP_STATUS.OK).json(
        new apiResponse(HTTP_CODE.OK,"Product Deleted SuccesFully")
    )
}
...
const ctrlUpdateProduct = async (req, res) => {
  ...
  const product = await Product.findByPk(req.body.id);
  if (!product) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND, "No Product Found With This Credentials");
  ...
  if (Object.keys(updatedData).length === 0 && (!req.files || req.files.length === 0)) {
    throw new apiError(HTTP_STATUS.BAD_REQUEST, "At least one field or image is required to update");
  }
  ...
  if (updatedData.product_title) {
    ...
    if (recordCheck) {
      throw new apiError(HTTP_STATUS.BAD_REQUEST, "Can't update record: product with this title already exists");
    }
  }
  ...
  if (req.files && req.files.length > 0) {
    ...
    if (currentImageCount >= 5) {
      throw new apiError(HTTP_STATUS.BAD_REQUEST, "You can only upload 5 images for a product");
    }

    const remainingSlots = 5 - currentImageCount;

    if (req.files.length > remainingSlots) {
      throw new apiError(HTTP_STATUS.BAD_REQUEST, `You can only upload ${remainingSlots} more images for this product`);
    }

    const productImages = req.files.map((file) => {
      if (!['image/png', 'image/jpeg', 'image/jpg'].includes(file.mimetype)) {
        throw new apiError(HTTP_STATUS.BAD_REQUEST, "Only PNG, JPG, and JPEG images are allowed");
      }
      return {
        product_id: product.id,
        image_path: file.path,
  ...
  const currentDate = new Date()
    const products = await Product.findAll({
        ...
        },
        {
          ...
          model:db.Discount,
          attributes : ['discount','startingDate','endingDate','typeOfDiscount'],
          where: {
          startingDate: { [Op.lte]: currentDate },
          endingDate: { [Op.gte]: currentDate }
        },
        required : false
        }],
    })
    if(!products) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND,"No Products Found")
  ...
  const currentDate = new Date()
    const product = await Product.findByPk(req.body.id,{
        include:[{
          ...
          model:db.Discount,
          attributes : ['discount','startingDate','endingDate','typeOfDiscount','is_active'],
          where: {
          startingDate: { [Op.lte]: currentDate },
          endingDate: { [Op.gte]: currentDate }
        },
...
const ctrlDeleteSingleProductImage = async(req,res)=>{
    ...
    if(!productImage) throw new apiError(HTTP_STATUS.DATA_NOT_FOUND,"No Product Image Found With This ID")
    
    await productImage.destroy()
    res.status(HTTP_STATUS.OK).json(
        new apiResponse(HTTP_CODE.OK,"Product Image Deleted SuccessFully")
    )
}
...
const ctrlUpdateProductImage = async(req,res)=>{
    ...
    const oldimagepath = path.join(__dirname, '..', '..',productImage.image_path)
    if(fs.existsSync(oldimagepath)){
        fs.unlinkSync(oldimagepath)
    }
    if(req.files && req.files.length > 0){
        if(req.files[0].mimetype !== 'image/png' && req.files[0].mimetype !== 'image/jpeg' && req.files[0].mimetype !== 'image/jpg') throw new apiError(HTTP_STATUS.BAD_REQUEST,"Only Png,Jpg And Jpeg Are Allowed")
    ...
    res.status(HTTP_STATUS.OK).json(
        new apiResponse(HTTP_CODE.OK,"Product Image Updated SuccessFully",productImage)
    )
}
